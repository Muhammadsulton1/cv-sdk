...
class BasicEventRegistrator(AbstractEventRegistrator):
    ...
    # TODO: Не добавлять единую точку для входа, при ошибки если закрыть редис, то остальные сервисы тоже упадут которые с ним связанны
    # TODO: Подумать о кэшировании кадра если один сервис инференса его скачал чтобы другой читал из кэша а не грузил его с TTL
